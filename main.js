...
const elBookMarkTemplate = document.querySelector(".js-bookMark-template").content;
...
const elCanvasList = document.querySelector(".offcanvas-body");
...
function renderMovies (arr, node) {
    ...
    arr.forEach(item => { 
        ...
        const youtubeThumbnailUrl = `https://img.youtube.com/vi/${item.ytid}/0.jpg`;
        templateCloneNodes.querySelector(".movie-poster").src = youtubeThumbnailUrl;
        templateCloneNodes.querySelector(".movie-title").innerHTML = searchInputVal ? highlightKeyword(item.Title, searchInputVal) :item.Title;
        templateCloneNodes.querySelector(".movie-rating").textContent = item.imdb_rating;
        templateCloneNodes.querySelector(".movie-premire").textContent = item.movie_year;
        templateCloneNodes.querySelector(".movie-duration").textContent = `${Math.floor(item.runtime / 60)} h ${item.runtime % 60} min`;
        templateCloneNodes.querySelector(".movie-genre").textContent = item.Categories.replaceAll("|", ", ");
        templateCloneNodes.querySelector(".js-moreInfo-button").dataset.imdbId = item.imdb_id;
        templateCloneNodes.querySelector(".js-bookMark-button").dataset.imdbId = item.imdb_id;
    ...
    if(evt.target.matches(".js-bookMark-button")) {
        ...
        movies.find(item => {
            if(item.imdb_id === btnImdbId) {
                ...
                const templateCloneNodes = elBookMarkTemplate.cloneNode(true);
                const youtubeThumbnailUrl = `https://img.youtube.com/vi/${item.ytid}/0.jpg`;
                templateCloneNodes.querySelector(".movie-poster").src = youtubeThumbnailUrl;
                templateCloneNodes.querySelector(".bookmark-list-item").dataset.imdbId = item.imdb_id;
                templateCloneNodes.querySelector(".movie-title").textContent = item.Title;
                templateCloneNodes.querySelector(".movie-rating").textContent = item.imdb_rating;
                templateCloneNodes.querySelector(".movie-premire").textContent = item.movie_year;
                templateCloneNodes.querySelector(".movie-duration").textContent = `${Math.floor(item.runtime / 60)} h ${item.runtime % 60} min`;
                templateCloneNodes.querySelector(".movie-genre").textContent = item.Categories.replaceAll("|", ", ");
                templateCloneNodes.querySelector(".js-remove-button").dataset.imdbId = item.imdb_id;

                elCanvasList.appendChild(templateCloneNodes);

                elCanvasList.addEventListener("click", function(evt) {
                    
                    if(evt.target.matches(".js-remove-button")) {
                        const btnImdbId = evt.target.dataset.imdbId;
                        
                        const canvasMovies = elCanvasList.querySelectorAll(".bookmark-list-item");
                        canvasMovies.forEach(canvasMovie => {
                            const movieImdbId = canvasMovie.dataset.imdbId;
                            if (movieImdbId === btnImdbId) {
                                canvasMovie.remove();
                            }
                        });
                    };
